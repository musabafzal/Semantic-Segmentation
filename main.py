...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    up_layer7 = tf.layers.conv2d_transpose(conv_1x1_layer7, num_classes, 4, 2, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    conv_1x1_layer4 = tf.layers.conv2d(vgg_layer4_out, num_classes,1, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer4_skip = tf.add(up_layer7, conv_1x1_layer4)

    up_layer4 = tf.layers.conv2d_transpose(layer4_skip, num_classes, 4, 2, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    conv_1x1_layer3 = tf.layers.conv2d(vgg_layer3_out, num_classes,1, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer3_skip = tf.add(up_layer4, conv_1x1_layer3)

    last_layer = tf.layers.conv2d_transpose(layer3_skip, num_classes, 32, 8, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, train_op, cross_entropy_loss = optimize(layer_ouput, label, learning_rate, num_classes)
